...
Local Mac gate at the time of writing: 93 tests green, lint clean, both
...
uv run pytest            # our tests only (testpaths = ["tests"]); 93 tests, ~5s, no API keys
...
`uv run pytest` must be green before any commit. As of HEAD: 81 passed in ~5s, no API keys
